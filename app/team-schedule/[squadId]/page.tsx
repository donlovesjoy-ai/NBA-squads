...
export default async function TeamSchedulePage({params}:{params:Promise<{squadId:string}>}){
  ...
  return <main style={{maxWidth:760,margin:'0 auto',padding:'24px 8px 60px'}}>
    ...
    {!rows.length?<div className="card" style={{textAlign:'center'}}>The 2026–27 NBA schedule has not been loaded yet.</div>:<section style={{background:'#fff',border:'1px solid #d1d5db',borderRadius:16,padding:'10px 8px 4px',overflow:'hidden'}}>
      <div style={{display:'grid',gridTemplateColumns:'42px minmax(92px,1.25fr) 70px 58px 44px',gap:3,alignItems:'end',padding:'7px 2px 10px',fontWeight:900,fontSize:10,borderBottom:'2px solid #e5e7eb'}}>
      ...
      {rows.map((g:any)=>{
        ...
        return <div key={g.id} style={{display:'grid',gridTemplateColumns:'42px minmax(92px,1.25fr) 70px 58px 44px',gap:3,alignItems:'center',padding:'9px 2px',borderBottom:'1px solid #e5e7eb',fontSize:11}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:5,minWidth:0,fontWeight:900}}>
            {opp?.logo_url&&<img src={opp.logo_url} alt={`${opp.name} logo`} style={{width:26,height:26,objectFit:'contain',flex:'0 0 auto'}}/>}
            <span style={{whiteSpace:'nowrap',fontSize:11}}>{home?'VS':'@'} {opp?.abbreviation||'TBD'}</span>
          </div>
